...
Каждый тест: транслирует `.asm` -> симулирует -> сравнивает `output_buffer` и усечённый журнал с файлами в `golden/`. При расхождении - точное сообщение о diff.

### Описание алгоритмов

**hello** - использует `PUSH_M` (читает длину), `PUSH_IND`/`POP_IND` (косвенная запись в MMIO-порт), цикл через `JZ`. Демонстрирует работу с pstr и прямым выводом.

**cat** - idle-loop на `JMP useless` ждёт прерывания. ISR: читает символ из порта 2045, проверяет на нуль-терминатор через `CMP`+`JZ`, выводит. Демонстрирует `trap` I/O и ISR.

**hello_user** - ISR накапливает символы в массив `name`, фиксирует конец по `\n`. Основная программа ждёт флага `input_done` (polling), потом печатает ответ. Демонстрирует взаимодействие ISR и основного кода.

**sort** - пузырьковая сортировка. Два вложенных цикла через `JZ`/`JNZ`. Обмен элементов через tmp-переменные + `POP_IND`. Вывод через decimal-порт. Формат массива: pstr-style (длина + элементы).

**double_math** - 64-битные операции: сложение через `ADD`+`ADC` (carry chain), вычитание через `SUB`+`SBC` (borrow chain), умножение через `MUL` (low 32) + `MULH` (high 32). Печать результатов через print_pstr.

**prob2** (Euler #6) - сумма 1..100 и сумма квадратов. Разность квадрата суммы и суммы квадратов. Вывод через decimal-порт. Демонстрирует `JNZ`, арифметику на стеке.

**reverse** - переворот строки `in-place` через два указателя (left/right), сходящихся к середине. Каждый шаг: загрузить два символа во временную переменную `tmp`, записать крест-накрест через `POP_IND`. После - печать. Демонстрирует: косвенную адресацию, `%ifdef`/`%define`.
